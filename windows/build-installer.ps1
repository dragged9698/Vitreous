...
if ($HasX64) {
    ...
    $X64Portable = Join-Path $ResolvedOutput "vitreous-windows-x64-portable.7z"
...
if ($HasArm64) {
    ...
    $Arm64Portable = Join-Path $ResolvedOutput "vitreous-windows-arm64-portable.7z"
...
if ($DualArch) {
    ...
    $IssContent = @"
#define Name "Vitreous"
#define Version "$Version"
#define Publisher "dragged9698"
#define ExeName "vitreous.exe"
...
AllowNoIcons=yes
OutputDir=.
OutputBaseFilename=vitreous-windows-installer
Compression=lzma
...
Write-Host "Installer:        $(Join-Path $ResolvedOutput 'vitreous-windows-installer.exe')" -ForegroundColor White
