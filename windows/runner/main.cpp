...
wWinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE prev, _In_ wchar_t* command_line, _In_ int show_command) {
  ...
  HANDLE mutex = CreateMutex(nullptr, TRUE, L"com.dragged9698.Vitreous.SingleInstance");
  if (GetLastError() == ERROR_ALREADY_EXISTS) {
    HWND existing = FindWindow(L"FLUTTER_RUNNER_WIN32_WINDOW", L"Vitreous");
    if (existing) {
  ...
  if (!window.Create(L"Vitreous", origin, size)) {
    return EXIT_FAILURE;
  }
