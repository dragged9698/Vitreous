...
/// Emby-shape helpers on top of the shared [ApiCache] substrate.
///
/// Cache rows for Emby item metadata use the compound connection id
...
class EmbyApiCache extends ApiCache {
  static EmbyApiCache? _instance;
  static EmbyApiCache get instance {
    if (_instance == null) {
      throw StateError('EmbyApiCache not initialized. Call EmbyApiCache.initialize() first.');
    }
    return _instance!;
  }

  EmbyApiCache._(super.db);
  ...
  /// usable `baseUrl`.
  ///
  /// Image paths are run through [EmbyImageAbsolutizer] so cached items
  /// carry the same absolute URLs as items produced by [EmbyClient]'s
  ...
  Future<MediaItem?> getMetadata(ServerId serverId, String itemId) async {
    ...
    try {
      ...
      final absolutizer = EmbyImageAbsolutizer(baseUrl: ctx.baseUrl, accessToken: ctx.accessToken);
      return EmbyMappers.mediaItem(
        data,
  ...
  ///
  /// [viewOffsetMs] is converted to Emby's 100-ns ticks for
  /// `UserData.PlaybackPositionTicks`. [lastViewedAt] is treated as Plex's
  /// epoch-seconds and translated to Emby's ISO-8601 `LastPlayedDate`.
  /// [viewedLeafCount] is ignored — Emby tracks per-show rollup via
  ...
  /// Load all pinned Emby metadata in a single query.
  ///
  ...
  Future<Map<String, MediaItem>> getAllPinnedMetadata() async {
    ...
    final absolutizers = <String, EmbyImageAbsolutizer>{};
    for (final id in entries.map((e) => e.serverId).toSet()) {
      final ctx = await _serverContext(id);
      if (ctx != null) {
        contexts[id] = ctx;
        absolutizers[id] = EmbyImageAbsolutizer(baseUrl: ctx.baseUrl, accessToken: ctx.accessToken);
      }
    }
    ...
    return await tryIsolateRun(() {
      ...
      for (final entry in entries) {
        ...
        try {
          ...
          final mapped = EmbyMappers.mediaItem(
            data,
            serverId: ServerId(ctx.machineId),
            serverName: ctx.name,
            absolutizer: absolutizer,
  ...
  ///
  /// `name` matches what the live [EmbyClient] stamps onto online
  ...
  /// [EmbyConnection.toConfigJson] writes, so cache-read absolutization
  ...
  Future<({String machineId, String name, String baseUrl, String accessToken})?> _serverContext(
    ...
    // `{machineId}/{userId}` (Emby). The compound match uses a
