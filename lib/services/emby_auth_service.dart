...
/// and entered in their Emby web UI to approve sign-in; the [secret] is
/// the opaque polling/exchange handle.
class EmbyQuickConnectInitiation {
  final String code;
  final String secret;
  const EmbyQuickConnectInitiation({required this.code, required this.secret});
}

/// Auth flow for adding or refreshing a [EmbyConnection].
///
/// Lifecycle for adding a server:
///   1. [probe] — validates the URL responds as a Emby server.
...
///      [EmbyConnection] ready to insert into [ConnectionRegistry].
///   3. (later) [validate] / [refresh] / [signOut] for the [ConnectionAuthService]
///      contract.
class EmbyConnectionAuthService implements ConnectionAuthService {
  EmbyConnectionAuthService({
    required this.clientName,
    required this.clientVersion,
    required this.deviceName,
    @visibleForTesting this._testHttpClientFactory,
  }) : _endpointDiscovery = EmbyEndpointDiscovery(testHttpClientFactory: _testHttpClientFactory);

  /// App identity sent in the `MediaBrowser` Authorization header. Emby
  ...
  final EmbyEndpointDiscovery _endpointDiscovery;
  ...
  /// URL is unreachable or doesn't look like a Emby server.
  Future<EmbyServerInfo> probe(String baseUrl) async {
  ...
  /// fully-formed [EmbyConnection]. Throws [MediaServerAuthException] for
  /// 401/403 responses; other transport errors propagate.
  Future<EmbyConnection> authenticateByName({
    ...
    try {
      final response = await client.post(
        ...
        timeout: MediaServerTimeouts.embyProbe,
      );
  ...
  /// failure — Emby <10.7 returns 404 on this path, and an offline server
  ...
  Future<bool> isQuickConnectEnabled(String baseUrl) async {
    ...
    try {
      final response = await client.get('/QuickConnect/Enabled', timeout: MediaServerTimeouts.embyProbe);
  ...
  Future<EmbyQuickConnectInitiation> initiateQuickConnect({
    required String baseUrl,
    required String deviceId,
  }) async {
    final normalised = _normaliseBaseUrl(baseUrl);
    ...
    try {
      // Current Emby (10.7+) accepts GET; older builds required POST.
      // Try GET first, fall back on 405.
      var response = await client.get('/QuickConnect/Initiate', timeout: MediaServerTimeouts.embyProbe);
      if (response.statusCode == 405) {
        response = await client.post('/QuickConnect/Initiate', timeout: MediaServerTimeouts.embyProbe);
      }
      if (response.statusCode == 401 || response.statusCode == 403) {
      ...
      return EmbyQuickConnectInitiation(code: code, secret: secret);
    } on MediaServerHttpException catch (e) {
      if (e.statusCode == 401 || e.statusCode == 403) {
  ...
  /// in their Emby web UI, then exchange the approved secret for a token
  /// and return a fully-formed [EmbyConnection]. Returns `null` on
  /// cancel, timeout, or server-side secret expiry (404 mid-poll). Throws
  /// [MediaServerAuthException] on auth failures (401/403).
  Future<EmbyConnection?> authenticateByQuickConnect({
    required String baseUrl,
    List<String>? baseUrls,
  ...
  Future<bool> validate(Connection connection) async {
    if (connection is! EmbyConnection) return false;
    final client = _authenticatedClient(connection);
    try {
      final response = await client.get(connection.embyCurrentUserPath, timeout: MediaServerTimeouts.embyProbe);
  ...
  Future<Connection> refresh(Connection connection) async {
    if (connection is! EmbyConnection) return connection;
  ...
  Future<void> signOut(Connection connection) async {
    if (connection is! EmbyConnection) return;
    final client = _authenticatedClient(connection);
    try {
      // Best-effort: server may already have invalidated the session.
      await client.post('/Sessions/Logout', timeout: MediaServerTimeouts.embySignOut);
    } catch (e) {
      appLogger.d('EmbyConnectionAuthService: signOut best-effort failed: $e');
    } finally {
      client.close();
    }
  }

  MediaServerHttpClient _authenticatedClient(EmbyConnection connection) {
    LogRedactionManager.registerToken(connection.accessToken);
    return _buildHttpClient(
      baseUrl: connection.baseUrl,
      headers: {
        'X-Emby-Token': connection.accessToken,
        'Authorization': buildEmbyAuthHeader(
          clientName: clientName,
          clientVersion: clientVersion,
          deviceName: deviceName,
  ...
  static String _normaliseBaseUrl(String input) => EmbyEndpointDiscovery.normalizeBaseUrl(input);

  /// Build a [EmbyConnection] from a successful auth/exchange response.
  /// Connection id is derived from `(machineId, userId)` so each user on a
  /// given server has a single stable connection row.
  static EmbyConnection _buildConnection({
    required EmbyServerInfo info,
