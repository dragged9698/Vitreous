...
mixin _EmbyPlaybackMethods on MediaServerCacheMixin {
  EmbyConnection get connection;
  FailoverHttpClient get _http;

  /// Backend-neutral [PlaybackExtras] for [itemId]. Emby exposes chapters
  ...
  Future<MediaSourceInfo?> fetchCachedMediaSourceInfo(String itemId) async {
    ...
    return embyMediaSourceToMediaSourceInfo(first, chapters: raw['Chapters'], trickplay: raw['Trickplay']);
  }
  ...
  Future<ScrubPreviewSource?> createScrubPreviewSource({
    ...
    return EmbyTrickplayService.create(
      client: this as EmbyClient,
      itemId: item.id,
  ...
  Future<List<MediaMarker>> _fetchMediaSegmentMarkers(String itemId) async {
    final endpoint = EmbyApiCache.mediaSegmentsEndpoint(itemId);
    try {
      return await fetchWithCacheFallback<List<MediaMarker>>(
            ...
            parseCache: embyMediaSegmentsToMarkers,
            parseResponse: (response) => embyMediaSegmentsToMarkers(response.data),
          ) ??
          const [];
    } on MediaServerHttpException catch (e) {
      if (e.statusCode != 404) {
        appLogger.d('EmbyClient.fetchPlaybackExtras media segments unavailable', error: e);
      }
      return const [];
    } catch (e) {
      appLogger.d('EmbyClient.fetchPlaybackExtras media segments unavailable', error: e);
  ...
  Future<List<MediaMarker>> _fetchCachedMediaSegmentMarkers(String itemId) async {
    try {
      final data = await cache.get(ServerId(cacheServerId), EmbyApiCache.mediaSegmentsEndpoint(itemId));
      return embyMediaSegmentsToMarkers(data);
    } catch (e) {
      appLogger.d('EmbyClient.fetchPlaybackExtras cached media segments unavailable', error: e);
      return const [];
    }
  }

  String _withApiKey(String urlOrPath) {
    final uri = EmbyImageAbsolutizer.joinUri(baseUrl: connection.baseUrl, urlOrPath: urlOrPath);
    final params = Map<String, String>.from(uri.queryParameters)..['api_key'] = connection.accessToken;
    return uri.replace(queryParameters: params).toString();
  }

  /// Emby playback URL resolution.
  ///
  /// Always POSTs `/Items/{id}/PlaybackInfo` so Emby can resolve external
  ...
  Future<PlaybackInitializationResult> getPlaybackInitialization(PlaybackInitializationOptions options) async {
    ...
    var mediaInfo = embyMediaSourceToMediaSourceInfo(
    ...
    final effectiveAudioStreamId = _resolveEmbyAudioStreamId(requestedAudioStreamId, mediaInfo);
    mediaInfo = _withSelectedEmbyAudioStream(mediaInfo, effectiveAudioStreamId);
  ...
  int? _validEmbyAudioStreamId(int? explicit, MediaSourceInfo mediaInfo) {
  ...
  int? _resolveEmbyAudioStreamId(int? explicit, MediaSourceInfo mediaInfo) {
    final validExplicit = _validEmbyAudioStreamId(explicit, mediaInfo);
  ...
  MediaSourceInfo _withSelectedEmbyAudioStream(MediaSourceInfo mediaInfo, int? selectedStreamId) {
  ...
  String? _embySubtitleFallbackPath(String itemId, String? mediaSourceId, MediaSubtitleTrack track) {
  ...
  List<SubtitleTrack> _buildExternalSubtitles(
    ...
    for (final track in mediaInfo.subtitleTracks) {
      ...
      final path = track.key ?? _embySubtitleFallbackPath(itemId, mediaSourceId, track);
      if (path == null) continue;
      // Emby's subtitle URL is a path relative to baseUrl; build the
  ...
  /// [sourceId] wins when present because Emby plugins may reorder merged
  ...
  Future<EmbyPlaybackBundle?> fetchPlaybackBundle(String itemId, {int sourceIndex = 0, String? sourceId}) async {
    final item = await fetchItem(itemId);
    final raw = item?.raw;
    if (raw is! Map<String, dynamic>) return null;
    final sources = raw['MediaSources'];
    if (sources is! List || sources.isEmpty) return null;
    final availableVersions = embySourcesToVersions(sources);
  ...
  /// can be omitted; for items with multiple versions Emby uses the
  ...
  String buildTrickplayTileUrl(String itemId, int width, int sheetIndex, {String? mediaSourceId}) {
    return buildEmbyTrickplayTileUrl(
      baseUrl: connection.baseUrl,
      accessToken: connection.accessToken,
      deviceId: connection.deviceId,
      itemId: itemId,
  ...
  Future<Map<String, dynamic>?> getPlaybackInfo(
    ...
    try {
      ...
      appLogger.w('EmbyClient: getPlaybackInfo failed', error: e, stackTrace: st);
      return null;
    }
  }
  ...
  Future<ExternalIds> fetchExternalIds(String itemId) async {
    ...
    if (providerIds is Map<String, dynamic>) {
      return ExternalIds.fromEmbyProviderIds(providerIds);
    }
    return const ExternalIds();
  }

  /// MPV opens streams outside [FailoverHttpClient]; send the same
  /// `MediaBrowser` identity headers the API client uses so reverse proxies
  /// accept `/Videos/*/stream` alongside the `api_key` query param.
  @override
  ...
  /// mirrors the Emby SDK's [PlaybackStartInfo] — Findroid sends the
  /// same fields, and Emby's session tracker drops events that omit
  ...
  /// Emby's `/Sessions/Playing` body has no slot for it. Stream indexes
  ...
  Future<void> reportPlaybackStarted({
    ...
    final response = await _http.post(
      ...
      body: {
        ...
        'AudioStreamIndex': ?audioStreamIndex,
        'SubtitleStreamIndex': ?subtitleStreamIndex,
        'PositionTicks': msToEmbyTicks(position.inMilliseconds),
        'CanSeek': true,
        'IsPaused': false,
        'IsMuted': false,
  ...
  /// audio/subtitle stream indexes when enabled in Emby user settings.
  @override
