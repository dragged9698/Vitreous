...
mixin _EmbyPlaylistMethods on MediaServerCacheMixin {
  ...
  Future<bool> deletePlaylist(MediaPlaylist playlist) async {
    // Emby treats playlists as items — same delete endpoint.
    final response = await _http.delete('/Items/${_segment(playlist.id)}');
    throwIfHttpError(response);
    return true;
  }

  /// Emby's move endpoint takes an absolute index, so [afterItem] is
