...
mixin _EmbyImageDownloadMethods on MediaServerCacheMixin {
  EmbyConnection get connection;
  Future<EmbyPlaybackBundle?> fetchPlaybackBundle(String itemId, {int sourceIndex = 0, String? sourceId});
  ...
  String thumbnailUrl(String? path, {int? width, int? height}) {
    ...
    final uri = EmbyImageAbsolutizer.joinUri(baseUrl: connection.baseUrl, urlOrPath: path);
  ...
  /// Emby doesn't expose an external-URL proxy endpoint comparable to
  ...
  Future<DownloadResolution> resolveDownload(MediaItem item, {int mediaIndex = 0}) async {
    ...
    // Direct-stream the selected original file. Emby's `Static=true`
  ...
  List<DownloadArtworkSpec> resolveDownloadArtwork(MediaItem item) {
    // Emby paths flow through `_absolutizeImagePath` at the mapper
