...
///
/// The real Emby web client + Findroid skip explicit `Fields` for
...
/// queries because it is the heaviest item field Emby returns.
...
/// Emby web's folder view requests none of them either. The unwatched
...
bool _isEmbyFolderDto(Map<String, dynamic> item) {
  final type = (item['Type'] as String?)?.toLowerCase();
  return type == 'folder' || type == 'collectionfolder' || (type == null && item['IsFolder'] == true);
}

String _embyFolderSortName(Map<String, dynamic> item) {
...
/// pre-fetch paths. Mirrors what the Emby web detail view requests.
const _detailFields =
    ...
    // Chapters: Emby returns them at the item level; the playback
    ...
    // unified tracker coordinator to scrobble Emby items without
    // any extra round-trip.
    'ProviderIds';

mixin _EmbyBrowseMethods on MediaServerCacheMixin {
  ...
  // Endpoint conventions follow what the official Emby Kotlin SDK
  ...
  // proxy rules and rate limiters as a stock Emby app.
  ...
  Future<List<MediaLibrary>> fetchLibraries() async {
    ...
    // Emby surfaces the user's collection (BoxSet) and playlist roots as
    ...
    return items
        ...
        .map((view) => EmbyMappers.library(view, serverId: serverId, serverName: serverName))
  ...
  /// Emby's `/Items/Filters` returns Genres / OfficialRatings / Tags /
  /// Categories + values from `/Items/Filters` in a single call. The unwatched
  /// boolean is synthetic because Emby exposes it as an `/Items` query
  ...
  /// prefixed `emby:` so FiltersBottomSheet can recognise it as cached and
  ...
  /// Emby internalisation of the Plex-style filter map → [LibraryQuery]
  ...
  /// rather than the recursive episode expansion Emby would otherwise
  /// produce.
  @override
  ...
  /// Synthesised 27-letter alphabet — Emby has no equivalent of Plex's
  ...
  /// Queue a metadata refresh for the library. Emby treats a library
  ...
  /// Emby has no single-round-trip equivalent of Plex's
  ...
  Future<MediaItem?> fetchItem(String id) async {
    ...
    try {
      ...
      try {
        ...
        appLogger.w('EmbyClient.fetchItem cache write failed', error: e, stackTrace: st);
      }
      ...
      appLogger.w('EmbyClient.fetchItem network call failed', error: e);
      try {
        final cached = await cache.get(ServerId(cacheServerId), endpoint);
        if (cached is Map<String, dynamic>) return _mapItem(cached);
      } catch (cacheError, st) {
        appLogger.w('EmbyClient.fetchItem cache fallback failed', error: cacheError, stackTrace: st);
      }
      rethrow;
    }
  }
  ...
  Future<List<MediaItem>> _fetchChildrenInternal(
    ...
    try {
      ...
      appLogger.w('EmbyClient.fetchChildren cache write failed', error: e, stackTrace: st);
    }
  ...
  /// Emby folder browsing mirrors Emby Web/Findroid/Swiftfin: query
  ...
  /// Contents of a Emby folder. Kept separate from [fetchChildren] so the
  ...
  Future<List<MediaItem>> _fetchFolderChildren(
    ...
    // makes Emby compute a recursive unplayed count PER FOLDER (measured
    ...
    allRaw.sort((a, b) {
      final folderRank = (_isEmbyFolderDto(a) ? 0 : 1).compareTo(_isEmbyFolderDto(b) ? 0 : 1);
      if (folderRank != 0) return folderRank;
      return _embyFolderSortName(a).compareTo(_embyFolderSortName(b));
    });

    try {
      await cache.put(ServerId(cacheServerId), cacheKey, {'Items': allRaw, 'TotalRecordCount': allRaw.length});
    } catch (e, st) {
      appLogger.w('EmbyClient.fetchFolderChildren cache write failed', error: e, stackTrace: st);
    }
    return _mapItems(allRaw);
  }
  ...
  /// Emby's generic `Video` / `MusicVideo` kinds for home-video libraries,
  ...
  Future<List<MediaItem>> fetchRecentlyAdded({int limit = 50}) async {
    // Matches userLibraryApi.getLatestMedia in the Emby SDK.
    final response = await _http.get(
  ...
  Future<List<MediaItem>> fetchContinueWatching({int? count = 20}) async {
    final results = await Future.wait([
      _fetchItemsArray(connection.embyResumeItemsPath, {
  ...
  Future<List<MediaHub>> fetchGlobalHubs({int limit = defaultHubPreviewLimit, bool includePlaybackHubs = true}) async {
    // Emby doesn't expose a single "hubs" endpoint, so we synthesise the
    ...
    final results = await Future.wait([
      latestFuture,
      _safeFetchItemsArray(connection.embyResumeItemsPath, {
        'Limit': limit.toString(),
        'Fields': _browseFields,
        'MediaTypes': 'Video',
        'Recursive': 'true',
  ...
  Future<List<MediaHub>> fetchLibraryHubs(
    ...
    // Mirror the Emby web client's per-library "Suggestions" tab:
    ...
    // Emby can otherwise spend time scanning TV state only to return [].
    final latestFuture = _safeFetchItemsArray('/Users/${_segment(connection.userId)}/Items/Latest', {
      'Limit': limit.toString(),
      'ParentId': libraryId,
      'Fields': _browseFields,
  ...
  Future<List<MediaItem>> fetchMoreHubItems(String hubId, {int? limit}) async {
    try {
      ...
      appLogger.w('EmbyClient: failed to fetch hub items for $hubId (treating as empty)', error: e, stackTrace: st);
      return const [];
    }
  }
  ...
  Future<LibraryPage<MediaItem>> fetchMoreHubItemsPage(
    ...
    switch (tail) {
      case 'recent':
        // Emby's Latest endpoint has a Limit but no StartIndex. Expose it
      ...
      case 'continue':
        return _safeFetchMediaPage(
          connection.embyResumeItemsPath,
          {
            'StartIndex': offset.toString(),
            'Limit': effectiveLimit,
            'Fields': _browseFields,
  ...
  Future<LibraryPage<MediaItem>> _safeFetchMediaPage(
    ...
    try {
      ...
      appLogger.w('EmbyClient: $path failed', error: e, stackTrace: st);
      rethrow;
    }
  }
  ...
  /// Emby's `/Shows/NextUp` returns the *next* (unwatched) episode for each
  ...
  Future<List<MediaItem>> _attachSeriesLastPlayed(List<MediaItem> nextUp) async {
    ...
    // newest play. We deliberately do NOT filter on the Played flag: Emby's
  ...
  /// Merge Emby's two continue-watching sources into one recency-ordered
  ...
  Future<List<Map<String, dynamic>>> _safeFetchItemsArray(
    ...
    try {
      ...
      appLogger.w('EmbyClient: $path failed (treating as empty)', error: e, stackTrace: st);
      return const [];
    }
  }

  /// Extras endpoints legitimately 404 when an item has no local trailers or
  /// special features — treat that as an empty list without log noise.
  Future<List<Map<String, dynamic>>> _fetchOptionalExtrasItems(
    ...
    try {
      ...
      appLogger.d('EmbyClient: $path failed (treating as empty)', error: e);
      return const [];
    } catch (e, st) {
      appLogger.d('EmbyClient: $path failed (treating as empty)', error: e, stackTrace: st);
      return const [];
    }
  }
}
