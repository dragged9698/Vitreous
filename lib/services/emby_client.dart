...
import '../models/emby/emby_user_profile.dart';
...
import 'emby_auth_header.dart';
import '../media/download_resolution.dart';
import 'api_cache.dart';
import 'download_artwork_helpers.dart';
import 'emby_api_cache.dart';
import 'emby_live_tv_dvr_mappers.dart';
import 'emby_mappers.dart';
...
part 'emby_client/parts/browse.dart';
part 'emby_client/parts/playback.dart';
part 'emby_client/parts/watch_state.dart';
part 'emby_client/parts/playlists.dart';
part 'emby_client/parts/collections.dart';
part 'emby_client/parts/file_info.dart';
part 'emby_client/parts/live_tv.dart';
part 'emby_client/parts/images_downloads.dart';
part 'emby_client/parts/metadata_edit.dart';

/// [MediaServerClient] over a Emby server.
///
/// Constructs from a [EmbyConnection] and a [MediaServerHttpClient] (the
...
class EmbyClient
    with
        ...
        _EmbyBrowseMethods,
        _EmbyPlaybackMethods,
        _EmbyWatchStateMethods,
        _EmbyPlaylistMethods,
        _EmbyCollectionMethods,
        _EmbyFileInfoMethods,
        _EmbyLiveTvMethods,
        _EmbyImageDownloadMethods,
        _EmbyMetadataEditMethods
    implements MediaServerClient, SeasonEpisodePagingClient, ScopedMediaServerClient, GracefullyCloseable {
  EmbyClient._({required this._connection, required this._http, FavoriteChannelsRepository? favoritesRepository})
    : _favoritesRepository = favoritesRepository ?? const SharedPreferencesFavoriteChannelsRepository();

  /// Build a fully-initialised [EmbyClient]. Endpoint reachability is
  ...
  /// every request — that's what the official Emby SDK (and Findroid by
  /// extension) does. Modern Emby servers behind reverse proxies often
  ...
  /// 401. We send `X-Emby-Token` too for old Emby/Emby builds.
  static Future<EmbyClient> create(
    ...
    final headers = {
      ...
      // Emby's session reporting endpoints (`/Sessions/Playing*`) reject
    ...
    client = EmbyClient._(connection: connection, http: http, favoritesRepository: favoritesRepository);
    return client;
  }

  /// Test-only factory that injects an [http.Client] so URL-builder tests
  /// can capture the request URI without spinning up a real Emby server.
  @visibleForTesting
  static EmbyClient forTesting({
    required EmbyConnection connection,
    ...
    client = EmbyClient._(connection: connection, http: mediaHttp, favoritesRepository: favoritesRepository);
    return client;
  }
  ...
  EmbyConnection _connection;
  @override
  EmbyConnection get connection => _connection;
  @override
  ...
  FutureOr<void> Function(EmbyConnection connection)? onConnectionUpdated;
  ...
  Future<void> _handleEndpointSwitch(String newBaseUrl, {required bool persist}) async {
    ...
    if (changed) {
      appLogger.i('Applying Emby endpoint switch', error: newBaseUrl);
  ...
  /// [EmbyApiCache] (which constructs its own from the connection row's
  /// `configJson`) so cache reads carry the same absolute URLs as live API
  /// reads — see [EmbyImageAbsolutizer].
  EmbyImageAbsolutizer get _absolutizer =>
      EmbyImageAbsolutizer(baseUrl: connection.baseUrl, accessToken: connection.accessToken);
  ...
  MediaItem? _mapItem(Map<String, dynamic> json) =>
      EmbyMappers.mediaItem(json, serverId: serverId, serverName: serverName, absolutizer: _absolutizer);
  ...
  @override
  ServerCapabilities get capabilities => ServerCapabilities.emby;

  /// Emby doesn't expose a per-server played-threshold pref, so we mirror
  /// Plex's default of 90%.
  @override
  double get watchedThreshold => 0.9;

  /// Emby marks an item played from `/Sessions/Playing/Stopped` itself
  ...
  Future<HealthStatus> checkHealth() async {
    try {
      final response = await _http.get(connection.embyCurrentUserPath, timeout: const Duration(seconds: 8));
            ...
            if (fresh != null && fresh != _connection.isAdministrator) {
              ...
              if (listener != null) {
                try {
                  ...
                  appLogger.w('Failed to handle Emby connection update', error: e, stackTrace: st);
                }
              }
            }
          }
  ...
  Future<EmbyUserProfile?> fetchUserProfile() async {
    try {
      final response = await _http.get(connection.embyCurrentUserPath);
      throwIfHttpError(response);
      final data = response.data;
      if (data is! Map<String, dynamic>) return null;
      return EmbyUserProfile.fromUserDto(data);
    } catch (e, st) {
      appLogger.w('EmbyClient.fetchUserProfile failed', error: e, stackTrace: st);
      return null;
    }
  }
  ...
  Future<String?> getMachineIdentifier() async {
    try {
      ...
      appLogger.w('EmbyClient: getMachineIdentifier failed: $e');
      return connection.serverMachineId;
    }
  }
