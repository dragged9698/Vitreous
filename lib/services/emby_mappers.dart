...
import 'emby_display_metadata.dart';
...
Map<String, dynamic>? embyFirstVideoStream(Object? streams) {
...
MediaVersion embyMediaSourceToVersion(
  ...
  final rawVideo = embyFirstVideoStream(source['MediaStreams']);
  ...
  return MediaVersion(
    ...
    parts: [
      MediaPart(
        ...
        container: source['Container'] as String?,
        durationMs: includePartDuration ? embyTicksToMs(source['RunTimeTicks']) : null,
        streams: streams,
      ),
    ],
    name: name,
...
/// Turns relative Emby image paths (e.g. `/Items/{id}/Images/Primary?tag=…`)
...
/// [EmbyClient].
class EmbyImageAbsolutizer {
  final String baseUrl;
  final String accessToken;
  const EmbyImageAbsolutizer({required this.baseUrl, required this.accessToken});
...
/// Pure mapping functions from Emby's `BaseItemDto` JSON shape into the
/// neutral [MediaItem] / [MediaLibrary] domain types.
///
/// Kept as top-level functions (no class) so they're trivially testable
/// against canned JSON fixtures and don't need a [EmbyClient] instance.
class EmbyMappers {
  EmbyMappers._();
  ...
  /// Map a Emby `BaseItemDto` (the `Items[]` shape returned by most
  ...
  static MediaItem? mediaItem(
    ...
    String? serverName,
    required EmbyImageAbsolutizer? absolutizer,
  }) {
    final id = item['Id'] as String?;
    if (id == null || id.isEmpty) return null;
    final type = item['Type'] as String?;
    // Untyped rows that Emby still flags as folders (defensive — typed
    ...
    final mapped = JellyfinMediaItem(
      ...
      originallyAvailableAt: embyIsoToYmd(item['PremiereDate'] as String?),
      ...
      // Episodes/seasons don't carry their own logo — Emby exposes the
      ...
      durationMs: embyTicksToMs(item['RunTimeTicks']),
      viewOffsetMs: embyTicksToMs(_userData(item)?['PlaybackPositionTicks']),
      viewCount: _viewCount(item),
      lastViewedAt: embyIsoToEpochSeconds(_userData(item)?['LastPlayedDate'] as String?),
      // Plex semantics: `leafCount` = total leaf items (episodes for series).
      // Emby's `ChildCount` is direct children (seasons for a series),
      ...
      addedAt: embyIsoToEpochSeconds(item['DateCreated'] as String?),
      updatedAt: embyIsoToEpochSeconds(item['DateLastSaved'] as String? ?? item['DateModified'] as String?),
      rating: (item['CommunityRating'] as num?)?.toDouble(),
      // Emby stores a binary `Likes` flag rather than a numeric rating.
  ...
  /// Map a Emby "view" (returned by `/Users/{userId}/Views`) into a
  ...
  static MediaLibrary? library(Map<String, dynamic> view, {required ServerId serverId, String? serverName}) {
    ...
    return MediaLibrary(
      ...
      updatedAt: embyIsoToEpochSeconds(view['DateLastSaved'] as String? ?? view['DateModified'] as String?),
      createdAt: embyIsoToEpochSeconds(view['DateCreated'] as String?),
      hidden: false,
      isShared: false,
      serverId: serverId,
      serverName: serverName,
  ...
  /// home-screen row (Emby doesn't have a single hub endpoint).
  ///
  /// [mapItem] lets the caller (typically [EmbyClient]) inject its own
  ...
  static String? preferredSubtitleLanguageFromItem(Map<String, dynamic> item) {
    final direct = item['PreferredSubtitleLanguage'];
  ...
  static List<MediaStream> _mediaStreams(Object? raw, {Map<String, dynamic>? source}) {
    ...
    for (final s in raw) {
      ...
      final isDolbyVision = isVideo && embyVideoStreamIsDolbyVision(s);
      result.add(
        MediaStream(
          id: '${f.index}',
          kind: kind,
          ...
          hdr: isVideo && embyVideoStreamIsHdr(source ?? const <String, dynamic>{}, s),
          dolbyVision: isDolbyVision,
          dolbyVisionProfile: isDolbyVision ? embyDolbyVisionProfile(s) : null,
          forced: f.isForced,
  ...
  static bool _embyStreamSelected(
  ...
  /// `ParentBackdropItemId`/`ParentBackdropImageTags` pair Emby
  ...
  /// `ParentLogoItemId`/`ParentLogoImageTag`. Match Emby web's hero
