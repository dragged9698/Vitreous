...
/// Translate a Emby `MediaSource` JSON object into [MediaSourceInfo] so the
...
/// up a [PlaybackInitializationService] or a [EmbyClient].
///
/// [chapters] is Emby's item-level `Chapters` array (the JSON list).
/// Each entry has `{Name, StartPositionTicks, ImageDateModified}`. Pass it
/// in here — Emby doesn't nest chapters inside MediaSource so the
...
/// `Map<resolutionKey, info>` (per Emby OpenAPI) and a nested
/// `Map<sourceId, Map<resolutionKey, info>>` (Streamyfin reports this).
MediaSourceInfo embyMediaSourceToMediaSourceInfo(
  ...
  // partId stays null for Emby because Plex's `/library/parts/{id}`
  // select-stream endpoint has no Emby equivalent. Emby track
  ...
  if (chapters is List) {
    for (var i = 0; i < chapters.length; i++) {
      ...
      final startMs = embyTicksToMs(entry['StartPositionTicks']) ?? 0;
  ...
  return MediaSourceInfo(
    ...
    partId: partId,
    displayCriteria: embyDisplayCriteriaFromStream(source, parsedStreams.videoStream),
...
/// Parse Emby chapters from the raw `BaseItemDto` payload into neutral
/// playback extras. Native Emby media segments are passed in separately;
/// chapter title fallback uses the same intro/credits patterns as Plex.
PlaybackExtras embyPlaybackExtrasFromRaw(
  dynamic raw,
  String itemId, {
  String? introPattern,
  ...
  final runtimeMs = raw is Map<String, dynamic> ? embyTicksToMs(raw['RunTimeTicks']) : null;
  ...
  if (chapters is List) {
    for (var i = 0; i < chapters.length; i++) {
      ...
      if (entry is! Map<String, dynamic>) continue;
      final startMs = embyTicksToMs(entry['StartPositionTicks']) ?? 0;
      // Emby chapter image path: `/Items/{itemId}/Images/Chapter/{i}?tag=...`.
...
List<MediaMarker> embyMediaSegmentsToMarkers(dynamic raw) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    final type = _embySegmentMarkerType(entry['Type']?.toString());
    final start = embyTicksToMs(entry['StartTicks']);
    final end = embyTicksToMs(entry['EndTicks']);
...
String? _embySegmentMarkerType(String? value) {
...
/// Coerce a Emby trickplay manifest to `Map<int width, TrickplayInfo>`,
...
/// Build a [MediaVersion] list from a Emby item's `MediaSources` array so
/// the existing version picker UI renders labels for alternate versions.
/// Resolution comes from the video stream inside `MediaStreams` — Emby
...
/// without spinning up a [PlaybackInitializationService] or a [EmbyClient].
List<MediaVersion> embySourcesToVersions(List<dynamic> sources) {
  final names = <String?>[];
